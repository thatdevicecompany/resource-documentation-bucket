...
A `PUT` request to `/groups/{id}/users` will add a set of users to the specified group. The body has
...
** <<resources-applications-list, Listing applications by user>>
...
** <<resources-bookmarks-list, Listing bookmarks by user>>
...
A `GET` request to `/bookmarks/search/findByGroupId?id=[group-id]` will list all of the groups in the group specified.
...
A `GET` request to `/bookmarks/search/findByUserId?id=[user-id]` will list all of the bookmarks in the user specified.
...
A `POST` request to `/bookmarks/{id}` will save a new bookmark and return the created resource.
