...
Only users with role ROLE_BW or ROLE_BR will see sponsors in this list.
...
A `POST` request to `users/{id}/picture` is used to generate an image representation that makes you able to upload a image file for the specified user.
...
A `GET` request to `/users/{id}/sponsors` will retrieve the list of users that are partner ot the user identified by `id`
...
defined at different levels: breezie level, <<organizations-micro-service, organization>> level and user level. Properties defined at
...
`Sensitive` is a representation of protected information of the user. Access to this resource is restricted.
...
[[sponsor-requests]]
=== Sponsor Request
A `sponsorRequest` represent an invitation to someone to become sponsor of a user
...
** <<sponsor-requests-email, Send sponsor request email>>
