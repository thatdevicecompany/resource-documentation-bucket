...
A `PUT` request to `/regions/{id}` is used to replace completely a region.
...
`devices`, `contents`... This resource is over the bottom of an pyramidal structure composed by `facilities`, `campuses`,
`regions` and `organizations`.
...
A `POST` request to `/campuses/{id}` will create a new campus and return the created resource metadata.
...
A `PUT` request to `/campuses/{id}` will do a complete replace of the campus.
...
A `POST` request to `/facilities{id}` will create a new facility and return the created resource metadata
...
A `PUT` request to `/facilities{id}` will do a complete replace of the facility.
