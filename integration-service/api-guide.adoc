...
[NOTE]
====
Connections can be created as a sub-resource of organizations, regions, campuses and facilities. The examples in this document
...
A `GET` request to `/organizations/{organizationId}/procura` will list all connections that belogs to the organization specified in the request.
This connection can be use for any administrators in the same organization or in any region, campur or facility that belongs to the organization.
...
A `GET` request to `/organizations/{organizationId}/procura/{id}` will retrieve the connection identified by `id`.
...
A `POST` request to `/organizations/{organizationId}/procura` will save a new connection and return the created resource. The private fields as `password`
and `apiKey` are not available for a request.
...
A `PUT` request to `/organizations/{organizationId}/procura/{id}` is used to update a connection completely.
...
A `PATCH` request to `/organizations/{organizationId}/procura/{id}` is used to update a connection partially.
...
A `DELETE` request to `/organizations/{organizationId}/procura/{id}` is used to delete a connection. This action will delete all the associations between
...
==== Associations
An association represents a link between a breezie user and a procura user made with a connection. An association has the
...
[[import-resource]]
==== Import
A import represents the action of importing users form Procura to Breezie. The action of importing users will be execute asynchronously. The status of
